...
static volatile uint8_t uarttx_idle_flag = 0;//UART TX 空闲状态描述符
__attribute__ ((aligned (4))) static uint8_t _usbtx_buffer[CONFIG_UARTRX_RINGBUF_SIZE];

// CherryRingBuffer 环形缓冲区的指针，用于操纵缓冲区的读写指针
...
void usbd_cdc_acm_bulk_in(uint8_t busid, uint8_t ep, uint32_t nbytes) {
    ...
    if ((nbytes % DAP_PACKET_SIZE) == 0 && nbytes) {//如果接收到的数据是DAP包大小的整数倍
        usbd_ep_start_write(busid, CDC_IN_EP, NULL, 0);
    } else {
        if (chry_ringbuffer_get_used(&g_uartrx)) {
            buffer = chry_ringbuffer_linear_read_setup(&g_uartrx, &size);
            //用于字节对齐
            memcpy(_usbtx_buffer, buffer, size);//将环形缓冲区中的数据拷贝到_usbtx_buffer
...
//将从设备收到的数据发送到USB，给电脑发送Uart收到的数据
...
void chry_dap_usb2uart_handle(void) {
    ...
    //uart_rx to usb_tx,将串口接收到的数据发送到USB
    if (usbtx_idle_flag) {//USB发送空闲
        if (chry_ringbuffer_get_used(&g_uartrx)) {//环形缓冲区中有数据
            usbtx_idle_flag = 0;
            buffer = chry_ringbuffer_linear_read_setup(&g_uartrx, &size);
            //用于字节对齐
    ...
    //usb_rx to uart_tx,将USB接收到的数据发送到串口
    if (uarttx_idle_flag) {
    ...
    if (usbrx_idle_flag) {//USB接收空闲
        if (chry_ringbuffer_get_free(&g_usbrx) >= DAP_PACKET_SIZE) {//环形缓冲区剩余空间大于DAP包大小
            usbrx_idle_flag = 0;
            usbd_ep_start_read(0, CDC_OUT_EP, usb_tmpbuffer, DAP_PACKET_SIZE);//启动USB读取数据到环形缓冲区
        }
    }
...
uint32_t get_cdc_g_line_coding_dwDTERate(void) {
    return g_cdc_lincoding.dwDTERate;
}

uint8_t get_cdc_g_line_coding_bCharFormat(void) {
    return g_cdc_lincoding.bCharFormat;
}

uint8_t get_cdc_g_line_coding_bParityType(void) {
    return g_cdc_lincoding.bParityType;
}
